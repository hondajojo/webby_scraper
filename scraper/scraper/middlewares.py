...
class BindAddressMiddleware(object):
    ...
    def process_request(self, request, spider):
        if self.is_bindaddress:
            if self.bindaddress:
                self.each_bindaddress = random.choice(self.bindaddress)
                spider.logger.info(self.each_bindaddress)
                request.meta['bindaddress'] = (self.each_bindaddress, 0)
        return None

    def spider_opened(self, spider):
        spider.logger.info('Using: %s as bindaddress' % self.each_bindaddress)
